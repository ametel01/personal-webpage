...
- Proof/content: seven original evergreen guides grounded in the site's existing project work and public evidence. Every guide includes a concrete diagram, implementation examples, decisions, failure cases, and direct repository or primary-reference links. No invented metrics, customers, or deployment claims.
...
- Memorable moment: a contour-line path leaves the featured guide and resolves into five topic waypoints; it becomes a simple vertical topic list on narrow screens.
...
| Topic connector | CSS/SVG geometry | One thin blue path and five circular waypoints; no canvas or raster. |
| Topic atlas | Semantic list of article links | Five real topics with descriptions and derived article counts; no inflated counts. |
| Supporting articles | Semantic linked rows | Six horizontal rule-separated rows with title, summary, topic, reading time, date, and directional arrow. |
| Article metadata | Existing Lucide icons + HTML | Clock and calendar icons with text labels; never icon-only. |
| Article detail | Semantic article | 65–75ch body, sticky TOC on wide screens, clear `h2`/`h3`, checklists, process diagram, real code, decision log, semantic failure-case descriptions, repository and primary references, and related project and writing links. |
